...
date:     18April2016,update:20April2016, update:23May2016, update:6June16, update:15June16, update:30June16, update:06Nov16
...
signatures_dir <- "~/Documents/PhDProjects/Multipathway_Modeling/Data" #delete later
...
# DELETE LATER THESE ARE SHELLEYS PATHS
icbp_status_file      <- paste(signatures_dir, "icbpstatus.txt", sep="/")
...
tcga_intrinsic_file   <- paste(signatures_dir, "TCGA_single_pathway_best_phenotype_intrinsic.txt", sep="/")
...
View(scaled_selected)

single_pathway_best_icbp[selected_celllines,]

#make the heatmap

par(mfrow=c(1,1),lwd=3)
heatmap.2(as.matrix(scale(single_pathway_best_icbp[selected_celllines,c(1,2,3,4,5,6,8)])),col=my_palette,trace="none",margins=c(7,7),main="",ColSideColors = c("coral3","aquamarine4","aquamarine4","coral3","coral3","aquamarine4","aquamarine4"), key.title="",scale="row", key=F)#,Rowv=F,Colv=F)
...
single_pathway_best_icbp_scaled[,c(1,4,5)]
single_pathway_best_icbp_scaled[,c(2,3,6:8)]
...
growth_icbp<-apply(single_pathway_best_icbp_scaled[,c(2,3,6:8)],1,mean)
...
{ 
  if(survival_icbp[[i]] > growth_icbp[[i]]){
    single_pathway_best_icbp_scaled[i,9] <- "Survival Phenotype"
  }
  else{
    single_pathway_best_icbp_scaled[i,9] <- "Growth Phenotype"
  }
}
...
boxplot2(her_pos$HER2,her_neg$HER2,ylab="HER2 Activity",ylim=c(0,1),las=1,names=c("HER2 +","HER2 -"),ylab="Estimated HER2 activity",main=paste("p-value: ",t.test(her_pos$HER2,her_neg$HER2)$p.value,sep=" "))
```
