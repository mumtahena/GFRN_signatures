...
plot_drug_boxplots_final <- function(drugdata, clusters, selected, fillcolor){
  ...
  theplot <- ggplot(merged_melt_s, aes(factor(clusters), value)) +
...
# TCGA cell line phenotypes from kmeans(4) clusters
...
Perform Principle Components Analysis in ICBP
```{r}
...
Perform Principle Components Analysis in TCGA
```{r}
...
Correlate TCGA Principle Components with ASSIGN generated GFRN pathway predictions
```{r}
...
Boxplot TCGA Principle Components across clinical subtypes (IHC: ER, PR, HER2)
```{r}
...
Boxplot TCGA Principle Components across BRCA intrinsic subtypes (PAM50)
```{r}
...
# Capitolize the Subtypes in ICBP 
